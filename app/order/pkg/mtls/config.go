...
func NewServerTLSConfig(caCertPath, certPath, keyPath string, clientVerify bool) (*tls.Config, error) {
	// 加载CA证书
	...
	if err != nil {
		return nil, fmt.Errorf("加载服务器证书和密钥失败: %v", err)
	}
...
func NewClientTLSConfig(caCertPath, certPath, keyPath, serverName string) (*tls.Config, error) {
	// 加载CA证书
	caCert, err := os.ReadFile(caCertPath)
	if err != nil {
		return nil, fmt.Errorf("读取CA证书失败: %v", err)
	...
	cert, err := tls.LoadX509KeyPair(certPath, keyPath)
	if err != nil {
		return nil, fmt.Errorf("加载客户端证书和密钥失败: %v", err)
	}
...
// NewKitexServerTLSConfig 创建Kitex服务端TLS配置
...
// NewKitexClientTLSConfig 创建Kitex客户端TLS配置
